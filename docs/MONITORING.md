...
| Payment/webhook failures | Backend logs for `Payme webhook error:` / `Stripe webhook error:` (`billingController.js`); Stripe Dashboard's own webhook delivery log; Payme's merchant portal | Any Stripe webhook marked failed/retrying, any Payme callback returning a non-success response |

...
import { FiBookOpen, FiEdit3, FiMessageSquare, FiPlus, FiTrash2, FiUsers } from 'react-icons/fi'
...
type Tab = 'courses' | 'users' | 'content' | 'moderation'
...
export default function ControlCenter() {
  ...
  return (
    <div className="atlas-page max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="admin-tabs mb-6">{([['courses', FiBookOpen, 'Courses'], ['users', FiUsers, 'People'], ['content', FiEdit3, 'Content'], ['moderation', FiMessageSquare, 'Moderation']] as const).map(([value, Icon, label]) => <button key={value} onClick={() => { setTab(value); if (value === 'content') loadContent() }} className={tab === value ? 'active' : ''}><Icon />{label}</button>)}</div>

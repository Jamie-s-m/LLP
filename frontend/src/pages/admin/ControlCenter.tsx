...
type Tab = 'courses' | 'users' | 'content' | 'applications' | 'moderation' | 'billing'
...
export default function ControlCenter() {
  ...
  return (
    <div className="atlas-page max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="admin-tabs mb-6">{([['courses', FiBookOpen, 'Courses'], ['users', FiUsers, 'People'], ['content', FiEdit3, 'Content'], ['applications', FiUserCheck, 'Applications'], ['moderation', FiMessageSquare, 'Moderation'], ['billing', FiCreditCard, 'Billing']] as const).map(([value, Icon, label]) => <button key={value} onClick={() => { setTab(value); if (value === 'content') loadContent(); if (value === 'applications') loadApplications(); if (value === 'moderation') loadContent(moderationResource) }} className={tab === value ? 'active' : ''}><Icon />{label}</button>)}</div>
      {tab === 'courses' && <section className="atlas-panel p-6"><div className="mb-6 flex flex-wrap justify-between gap-4"><div><p className="atlas-kicker">Content inventory</p><h2 className="text-2xl text-ink dark:text-white">Courses</h2></div><button onClick={() => openCourseModal()} className="btn btn-primary inline-flex items-center gap-2"><FiPlus /> New Course</button></div>{loading ? <div className="rounded-2xl bg-[#f6efe7] p-5 text-slate-600 dark:bg-white/5 dark:text-slate-300">Loading curriculum...</div> : <div className="admin-table">{courses.map((course) => <div className="admin-row" key={course._id}><div><strong>{course.title}</strong><small>{course.language} · {course.level} · {course.totalLessons || 0} lessons</small></div><span className={`status-pill ${course.isPublished ? '' : 'muted'}`}>{course.isPublished ? 'Published' : 'Draft'}</span><button onClick={() => openCourseModal(course)} className="icon-button" aria-label={`Edit ${course.title}`}><FiEdit3 /></button><button onClick={() => removeCourse(course._id)} className="icon-button danger" aria-label={`Delete ${course.title}`}><FiTrash2 /></button></div>)}{courses.length === 0 && <div className="empty-state"><FiBookOpen /><p>No courses yet. Create your first course to populate the catalog.</p></div>}</div>}</section>}
      {tab === 'users' && <section className="atlas-panel p-6"><div className="mb-6 flex flex-wrap justify-between gap-4"><h2 className="text-2xl text-ink dark:text-white">People & roles</h2><input className="input max-w-sm" value={userSearch} onChange={(event) => { setUserSearch(event.target.value); setUserPage(1) }} placeholder="Search name, email, or role" /></div><div className="admin-table">{visibleUsers.map((user) => <div className="admin-row" key={user._id}><div><strong>{user.firstName} {user.lastName}</strong><small>{user.email} · {user.role}</small></div><span className={`status-pill ${user.isActive ? '' : 'muted'}`}>{user.isActive ? 'Active' : 'Suspended'}</span><button onClick={() => toggleUser(user)} className="icon-button" aria-label={`Toggle ${user.email}`}><FiEdit3 /></button></div>)}{visibleUsers.length === 0 && <div className="empty-state"><FiUsers /><p>{filteredUsers.length === 0 ? 'No users match this search yet.' : 'No users on this page.'}</p></div>}</div><div className="mt-5 flex items-center justify-between text-sm text-slate-600 dark:text-slate-300"><span>{filteredUsers.length} users</span><div className="flex gap-2"><button className="btn btn-outline" disabled={userPage === 1} onClick={() => setUserPage((page) => page - 1)}>Previous</button><span className="px-3 py-2">{userPage} / {totalUserPages}</span><button className="btn btn-outline" disabled={userPage === totalUserPages} onClick={() => setUserPage((page) => page + 1)}>Next</button></div></div></section>}
      {tab === 'content' && <section className="atlas-panel p-6"><div className="mb-6 flex flex-wrap items-center justify-between gap-4"><h2 className="text-2xl text-ink dark:text-white">Content library</h2><select className="input max-w-xs" value={resource} onChange={(event) => { setResource(event.target.value); loadContent(event.target.value) }}><option value="lessons">Lessons</option><option value="flashcards">Flashcards</option><option value="posts">Forum posts</option><option value="groups">Groups</option></select></div><div className="admin-table">{content.map((item) => <div className="admin-row" key={item._id}><div><strong>{item.title || item.name || item.content?.slice(0, 70) || 'Untitled item'}</strong><small>{resource} · {item._id}</small></div><button onClick={() => removeContent(item._id)} className="icon-button danger" aria-label="Delete content"><FiTrash2 /></button></div>)}{content.length === 0 && <div className="empty-state"><FiEdit3 /><p>No items in this collection.</p></div>}</div></section>}
      {tab === 'applications' && <section className="atlas-panel p-6"><h2 className="mb-6 text-2xl text-ink dark:text-white">Teacher applications</h2><div className="admin-table">{applications.map((applicant) => <div className="admin-row" key={applicant._id}><div><strong>{applicant.firstName} {applicant.lastName}</strong><small>{applicant.email}</small></div><button onClick={() => reviewApplication(applicant._id, true)} className="icon-button" aria-label={`Approve ${applicant.email}`}><FiCheck /></button><button onClick={() => reviewApplication(applicant._id, false)} className="icon-button danger" aria-label={`Reject ${applicant.email}`}><FiX /></button></div>)}{applications.length === 0 && <div className="empty-state"><FiUserCheck /><p>No pending teacher applications.</p></div>}</div></section>}
      {tab === 'moderation' && <section className="atlas-panel p-6"><div className="mb-6 flex items-center justify-between gap-4"><h2 className="text-2xl text-ink dark:text-white">Community moderation</h2><select className="input max-w-xs" value={moderationResource} onChange={(event) => { const next = event.target.value as 'posts' | 'groups'; setModerationResource(next); loadContent(next) }}><option value="posts">Forum posts</option><option value="groups">Groups</option></select></div><div className="admin-table">{content.map((item: any) => <div className="admin-row" key={item._id}><div><strong>{item.title || item.name || 'Untitled item'}</strong><small>{item.category || moderationResource} {item.isPinned ? '· Pinned' : ''}</small></div>{moderationResource === 'posts' && <button onClick={() => togglePinnedPost(item)} className="icon-button" aria-label="Toggle pinned status"><FiCheck /></button>}<button onClick={() => removeContent(item._id)} className="icon-button danger" aria-label="Delete community item"><FiTrash2 /></button></div>)}{content.length === 0 && <div className="empty-state"><FiMessageSquare /><p>No moderation items.</p></div>}</div></section>}
      {tab === 'billing' && <section className="atlas-panel p-6"><div className="mb-6"><p className="atlas-kicker">Commercial controls</p><h2 className="text-2xl text-ink dark:text-white">Billing readiness</h2><p className="mt-2 text-muted">Use this checklist before you start charging live customers.</p></div><div className="grid gap-4 lg:grid-cols-3">{[
      ...
      ].map((card) => <div key={card.title} className="rounded-2xl bg-[#f6efe7] p-5 dark:bg-white/5"><p className="text-sm font-semibold text-slate-500 dark:text-slate-400">{card.title}</p><strong className="mt-3 block text-3xl text-ink dark:text-white">{card.value}</strong><p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{card.note}</p></div>)}</div><div className="mt-6 grid gap-4 md:grid-cols-2">{[
      ...
      ].map((item) => <div key={item} className="rounded-2xl border border-slate-200 bg-white/80 p-4 text-sm text-slate-700 dark:border-white/10 dark:bg-white/5 dark:text-slate-200">{item}</div>)}</div></section>}
